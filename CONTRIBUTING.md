...
- PR は CI(typecheck、lint、test、schema の差分、依存の監査、Semgrep と CodeQL による静的解析)が通ることが条件です。手元では `bun run typecheck`、`bun run lint`、`bun test` を回してください
...
- A PR must pass CI (typecheck, lint, test, schema diff, dependency audit, and static analysis with Semgrep and CodeQL). Locally: `bun run typecheck`, `bun run lint`, `bun test`.
