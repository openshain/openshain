...
## 文脈が増える一方であることは、いまの版の限界

決めたこと。会話は毎ターン記録から組み立て直します。減るのは、作業の Work を閉じたときにその Tool の結果を畳む分だけです。人の発言と返答は増え続け、いずれ model の窓を超えます。次の版で、要約をイベントとして記録し、投影がそこから先を組み立てる形を入れます(spec/interactive-cli.md)。

理由。記録が原本で、投影はその関数だと決めています。要約をその場の作業として持つと、同じ記録から同じ投影ができるという規則が崩れます。要約もイベントにすれば、規則を保ったまま入力を減らせます。窓を超えたときの失敗が、そのセッションでは以後ずっと起きることは、いまの限界として認めています。
