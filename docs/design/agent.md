...
名前。セッションを開くたびに、製品が持つ架空の名前の一覧(かなの名、30 個)から 1 つ選び、`work.created` の `agent_name` に残します。子 Work にも同じ名前を渡すので、質問のときも同じ名で話します。開いているセッションが使っている名前は避けます。設定や職種に名前を置かないのは、同じ職種のセッションが並行して開くことがあるからで、model に選ばせないのは、呼び出しが 1 回増えるうえ、投影は記録から組み立てるので結局記録に残す必要があり、実在の人名を選ぶ危険も避けにくいからです。記録にあるので、セッションの再開(未実装)でも同じ名前で続けられます。
