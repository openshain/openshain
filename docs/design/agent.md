...
決めたこと。直前のモデル呼び出しが実際に使った入力トークンが閾値を超えていたら、次のターンを始める前に 1 回だけ会話を要約します。要約は `conversation.compacted` として記録し、投影はそこから先を組み立てます。閾値は `limits.compact_at_input_tokens`、書かないときは `model.context_tokens` の 70%、それも無ければ 150000 です。要約のうち「引いた会社の決まり」と「実行できなかった呼び出し」の 2 節は、モデルではなく loop が書きます。要約が空のとき、置き換える対象より大きいとき、鍵らしき文字列が入っているとき、呼び出しが失敗したときは記録せず、そのターンをそのまま続けます。縮まなかったときは、次のターンで自動では試しません。

理由。見積もりではなく実測値を使うのは、トークンの数え方がモデルごとに違うからです。閾値をモデル名から決めないのは、OpenAI 互換の経路に素性の分からないモデルが来るからで、長さを知っているのは人だけです。それでも閾値が実際と合わないことはあるので、呼び出しが「入力が大きすぎる」(`too_large`)で失敗したときは、要約して 1 回だけやり直します。閾値は当てにならなくても、この経路があれば会話は詰みません。ターンの途中で要約しないのは、多段の Tool 呼び出しの間は作業の Work が開いたままで、その記録は会話の側が要約するものではないからです。決まりと却下の 2 節を loop が書くのは、決まりの本文を要約に写すと人ごとの絞り込みが素通りし、却下の理由が落ちると人が経緯を忘れたまま同じ承認を決めるからです。鍵らしき文字列を含む要約を記録しないのは、元の Tool の結果なら会話から流れて消えるのに、要約に入ると以後ずっと先頭に残るからです。取りこぼしはありますが、見つけたものを通さないだけでも、残り続ける経路は塞げます。
