...
決めたこと。判定が保留した呼び出しには、そのとき path guard が解決した場所を `approval.requested` に残します。承認して実行するときに同じ入力をもう一度解決し、違う場所を指していれば実行せず、`tool.rejected`(`path_changed`)を残します。資格者が入力を書き換えたとき(`modify`)も同じ検査を通ります。

理由。承認は会話をまたぎ、次の日に決められることもあります。その間に、会社フォルダの中の link や途中のフォルダが入れ替わると、同じ入力が別の場所を指します。人は画面で見た場所に対して承認したので、書く場所が変わったなら実行しないほうが正しく、黙って別の場所に書くのは最も避けたい結果です。
