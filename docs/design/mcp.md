...
決めたこと。`work_select` は、その Work の principal が設定の principal と違うとき受け付けません。`work_record` は「その接続で作ったか選んだ Work」にしか書けないので、選ぶところを塞げば、client が別の人の会話にイベントを書く経路はなくなります。読み取り(`work_get`、`work_list`)は、`reads` を書いた人には自分の Work だけを返します。`reads` を書いていない人は、会社フォルダのどの Work も読めます。

理由。会話の記録は、その人が何を頼み、何を了承したかの原本です。`work/` は予約パスなので Tool からは書けず、`work_record` が client にとって唯一の書き込み口です。ここに人の検査が無いと、同じ会社フォルダに接続した別の client が、他人の会話に発言や要約を書けます。読み取りを同じ範囲で絞るのは、`work/` が予約パスであることの裏返しです。社員エージェントはそこを `fs_read` では読めず、`work_list` と `work_get` が記録への唯一の経路なので、ここを開けたままにすると、範囲の外のファイルの path も、他の人の依頼の文も、承認待ちの呼び出しの入力も、そこから読めます。読めない Work には、無い Work と同じ返事を返します。別の返事にすると、その id に誰かの Work があることを伝えるからです。会社フォルダを直接読める人は `work/` の中の JSON をそのまま読めるので、これは人に対する機密ではなく、openshain を通る経路の筋を通すものです。
...
対話型 CLI の model には、この 2 つを渡しません(承認と判断の Tool と同じ扱い)。止めるのは人の操作で、その人は画面の前にいます。社員エージェントが読むファイルには「この作業を止めろ」と書けるので、一覧に無ければその文は呼ぶ先を持ちません。外の client には渡します。止めるのは外からだからです。

止めた Work も、承認そのものは持ったままです。`approval_decide` と `review_decide` は、承認が無いのではなく**止まっているから実行できない**と答えます。「承認がありません」と答えると、止めた本人が消えた承認を探すことになります。
...
決めたこと。判定が保留した呼び出しには、そのとき path guard が解決した場所と、**そのときその場所にあったファイルの sha256**(無ければ null)を `approval.requested` に残します。承認して実行するときに、同じ入力をもう一度解決し、ファイルをもう一度読み直します。場所が違えば `tool.rejected`(`path_changed`)、中身が違えば `tool.rejected`(`base_changed`)を残して実行しません。資格者が入力を書き換えたとき(`modify`)も同じ検査を通ります。中身を見るのは、書き込む Tool(`effect: mutate`)で path を持つ呼び出しだけです。

理由。承認は会話をまたぎ、次の日に決められることもあります。その間に、会社フォルダの中の link や途中のフォルダが入れ替わると、同じ入力が別の場所を指します。同じ間に、書き込む先のファイルの中身も変わります。人が自分で編集した、同期サービスが新しい版を落とした、別のセッションが書いた、のどれもです。人は画面で見た差分に対して承認したので、その差分の前提が変わったなら実行しないほうが正しく、黙って上書きするのが最も避けたい結果です。model には、読み直して提案し直すよう理由の文で伝えます。

捨てた案。書き込む Tool が書く代わりに提案を返し、Runtime が保存した提案を適用する形(ChangeSet)。承認した入力はいまも保存されていて、それをそのまま実行しているので、承認と実行の一致はこの検査で足ります。提案を保存する形が要るのは、第三者の Tool にも差分を持たせるとき、または model の出力を通さずに中身を作る Tool を入れるときです。どちらもまだありません。
