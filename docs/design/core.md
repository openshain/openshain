...
決めたこと。投影を短くする手段を 2 つ、`buildProjection` の中に置きます。1 つは要約です。`conversation.compacted` があれば、そのイベントが覆う範囲(`through` が指すイベントまで)を要約の本文 1 つに置き換え、そこから後を続けます。もう 1 つは古い Tool の結果です。直近 5 件の人の発言より前の `tool.completed` は、本文を「省略」の 1 行に置き換えます。呼び出しと結果の対はそのまま残します。実行しなかった呼び出し(`tool.rejected`)は、どれだけ古くても理由を残します。要約が呼び出しを覆い、その結果だけが後に残ったときは、結果を落とします。

理由。どちらも記録には触れません。イベントは全部そこにあり、投影はその関数のままです。要約が覆う範囲を `seq` ではなく `id` で指すのは、`seq` が Work ごとの採番で、会話の位置を指す番号として使うと意味が二重になるからです。要約に断り書きを付けるのは、ファイルや検索の結果に紛れた命令めいた文が要約に残っても、それが指示として読まれないようにするためです。古い結果を省略するのは、`foldAway` が Work の中の呼び出しにしか効かないからです。Work を作らずに答える呼び出し(`context`、`work_list`、`knowledge_search`)の結果は、どの Work も閉じないので会話に残り続けます。却下の理由を残すのは、落ちると人が経緯を忘れたまま同じ承認を決めることになるからです。覆われた呼び出しの結果を落とすのは、要約と要約の間ではなく、呼び出しと結果の間で要約が書かれることがあるからです(社員エージェントの質問と、人の答えの間)。答える相手のいない結果は provider が受け付けないので、投影に入れません。何があったかは要約の側にあります。
...
決めたこと。`measureWork` は 1 件の Work について 5 つのまとまり(coverage、intervention、quality、cost、attention)を、その Work のイベントだけから数えます。記録の無いものは 0 と書かず、項目を置きません。

理由。0 は「起きなかった」であって「分からない」ではありません。人が openshain の外でやった作業を 0 と書けば、手作業は無かったことになります。専門家に払った金額を 0 と書けば、費用は model のぶんだけになります。どちらも、あとで North Star を組み直すときに嘘の土台になります。原典(§36、Vol.2 §35)が挙げる 15 項目のうち、いま出どころがあるのは 7 つです。残りは、その出来事を記録する仕組みを入れてから数えます。

`review.decided` に、資格者が承認したのか書き換えたのかを足しました。それまで記録には結果しか残らず、**資格者が社員エージェントの提案を直したかどうかが読めませんでした**。quality の「直された回数」はそこから数えます。telemetry の都合ではなく、記録が原本であるための穴です。

費用の集計は core に 1 つだけ置きます。CLI が同じ足し算を別に持っていたので、そちらは言葉の組み立てだけにしました。同じ数を 2 か所で作ると、いつか食い違います。

attention は、いまは intervention の合計です。通知の仕組みが入るまでは、同じものを人の側から数えたものだからです。

## lock は、誰が持っているかと、どこで取ったかを書く

決めたこと。Work の lock ファイルに、取ったプロセスの id と時刻に加えて、**取った場所(そのフォルダの絶対パス)** を書きます。lock を見つけたとき、書かれた場所がいま自分がいる場所と違えば、それは別のフォルダから運ばれてきたものとして扱い、取り直します。場所が同じなら、これまでどおりプロセスが生きているかどうかで決めます。場所が書かれていない古い lock も、これまでどおりに扱います。

理由。会社フォルダは持ち運べることが前提です(合格条件 7)。同期サービスは、会話が開いていても構わずフォルダを複製します。そのとき lock も一緒に運ばれ、そこに書かれたプロセスは**元のフォルダのほうで**生きています。場所を見ないと、運んだ先ではその Work を二度と開けません。実際に再現しました。lock が守るのは 1 つのフォルダの中の単一書き込みなので、別のフォルダで取られた lock は、この フォルダについて何も言っていません。

捨てた案。プロセス id だけで判定したまま、運んだ先では人が lock ファイルを消す。持ち出しのたびに手作業が要り、消してよい lock かどうかを人が判断することになります。

変える条件。フォルダを開いたまま rename すると、書かれた場所と一致しなくなるので、別のプロセスが lock を取れます。同じ inode に 2 つの書き手が並ぶ形ですが、動いている会社フォルダの名前を変えるのは通常の操作ではないので、運べないことの害のほうを重く見ました。
