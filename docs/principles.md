...
| Paid means easiest | OSS だけで主要な用途を完了します。有償で提供するものがあるなら、それは運用の代行で、機能の解禁ではありません |
...
| Bring your own capability | ブラウザ操作などの実装は利用者が選びます |
| Own governance, not browser engines | ブラウザ操作などの実行手段そのものより、それをいつ、誰が、何のために使うかを管理します |
