...
承認(`authority/` の `approval_required`)について、いまの版の限界です。承認するのは接続が代理する principal で、その人が本当に操作したかを Runtime は確かめられません。対話型 CLI では人が画面で選びますが、MCP で接続した外部のエージェント(Claude Code など)は、自分が止められた呼び出しを自分で承認できます。承認を人の関門として使うなら、外部のエージェントには承認の要る規則を任せず、対話型 CLI で決めてください。端末の認証は後の版で入れます。
