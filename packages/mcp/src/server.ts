...
/** The tools every session has, before the workspace's own. */
const WORK_TOOLS: Tool[] = [
  {
    name: "work_create",
    description:
      "Start a work for a request from the person you work for, and make it the current work. Tool calls are recorded against the current work.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "work_complete",
    description:
      "Finish the current work. Say what was done; name the files you produced with their sha256 if you know it. The runtime checks every file and records its own hash.",
    inputSchema: {
      type: "object",
      properties: {
        summary: { type: "string" },
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      switch (name) {
        ...
        case "work_select": {
          const id = parseWorkId(String((input as { id?: unknown }).id ?? ""));
        ...
        case "work_get": {
          ...
          const id = given ? parseWorkId(String(given)) : session.current;
        ...
        case "work_complete": {
          ...
          const work = await complete(works, workspaceRoot, id, summary, artifacts ?? []);
          session.clear();
          return json(work);
        }
        case "work_fail": {
          ...
          const handle = await works.open(id);
          try {
            await handle.append({ type: "work.failed", payload: { reason, detail: detail ?? "" } });
          } finally {
            await handle.close();
          }
          session.clear();
        ...
        default: {
          ...
          const handle = await works.open(id);
          try {
            const result = await callTool(handle, { id: newCallId(), name, input });
...
/** Records the evidence and the completion. Artifacts named by the agent join the ones the tools wrote; the runtime hashes them all. */
async function complete(
  ...
  try {
    ...
    if (isTerminal(current.status)) {
      throw new OpenshainError("invalid_transition", `work ${id} is already ${current.status}`);
    }
