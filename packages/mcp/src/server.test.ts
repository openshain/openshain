...
`,
  ...
  test("what a person's work does not cover is not listed, not searched, not read, not written", async () => {
    ...
    const fixtures = join(import.meta.dir, "..", "..", "tools", "fixtures");
    await writeFile(
      join(root, "hr", "offer.pdf"),
      await readFile(join(fixtures, "pdf/invoice.pdf")),
    );
    await writeFile(
      join(root, "hr", "pay.xlsx"),
      await readFile(join(fixtures, "excel/ledger.xlsx")),
    );
    await writeFile(join(root, "principals", "alice.yaml"), "id: alice\nname: Alice\n");
    await writeFile(
    ...
    // Every tool that names a path is judged at the same door, the new ones included.
    const pdf = await asBob("pdf_read", { path: "hr/offer.pdf" });
    const book = await asBob("excel_read", { path: "hr/pay.xlsx" });
    ...
    expect(book.isError).toBe(true);
    expect(book.text).toContain("働く範囲の外");
