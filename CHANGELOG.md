...
- 観測と義務。`openshain observe <type>` で起きたことを `observations/` に記録し、`obligations/` に書いた義務に当たれば Work を作ります。Work には、どの観測から、どの義務で作られたかが残ります。観測は書き換えません(事実は後から変わりません)。この版の引き金は観測だけで、時刻と条件の引き金は後の版です。`observations/` と `obligations/` は予約パスで、Tool からは読み書きできません
- **`work.created` に `observation` と `obligation` が増えました。** 誰にも言われずに始まった Work が、なぜ在るかを持ちます
- 1 件の Work を測れるようになりました。終わったか、人を呼ばずに終わったか、質問と承認と資格者の判断の回数、資格者に書き換えられた回数、規則が止めた回数、model と Tool の使用量。`openshain work show` の末尾に 1 行で出ます。**記録の無いものは 0 と書かずに出しません。** 0 は「起きなかった」であって「分からない」ではないためです(openshain の外での手作業、専門家への支払、通知の回数はまだ記録がありません)
- **`review.decided` に `decision` が増えました。** 資格者が承認したのか、実行しないと決めたのか、入力を書き換えたのかが記録に残ります。これまでは結果しか残らず、提案が直されたかどうかを読めませんでした
- `work_pause` と `work_resume`。Work を外から止めて、また続けられます。止まっている間は、その Work に対する Tool の呼び出しも、人が既に承認した呼び出しの実行も、資格者の判断による実行も受け付けません。対話型 CLI の社員エージェントには渡しません(止めるのは人の操作です)。続けるときは、止めたときにやっていたこと(進行中、入力待ち、承認待ち)に戻ります
- **`WorkStatus` に `paused` が増えました。** `Record<WorkStatus, ...>` を書いている実装は追加してください

### Fixed

- 会話を開いたまま同期された会社フォルダで、その Work を開けるようになりました。これまでは Work の lock が一緒に複製され、そこに書かれたプロセスが元のフォルダで生きているため、運んだ先ではその Work を二度と開けませんでした。lock に「どこで取ったか」を書き、別のフォルダで取られた lock は取り直します

### Changed

- **`Observation` という型名が、Vol.1 の Fundamental Object のものになりました。** Tool の結果に付く出どころは `ToolObservation` です。項目名(`observation`)は変わらないので、第三者の Tool の動きは変わりません
- 成果物と書き込み先のハッシュを、ファイル全体をメモリに載せずに取ります。どちらも path を決めるのは model なので、名指しされた大きさをそのまま抱えない形にしました

### Added

- `excel_read`。`.xlsx` の 1 シートを、`csv_read` と同じ形で読みます。シートの名前の一覧、列名、全行数、行の窓を返します。日付は `YYYY-MM-DD` に直します(Excel は日付を `46204` のような数値で持つので、そのままでは日付だったことが分かりません)。数式のセルは Excel が保存した計算結果を返します。列名が空のセルと重複した名前は、その列の記号(A、B、C)にします。展開が大きすぎる `.xlsx` は開きません。古い `.xls` は読めないと返します
...
- `pdf_read`。会社フォルダの PDF から文字を取り出します。解析するのは頼まれたページだけで、pdf.js は eval の経路を閉じて開きます(PDF は取引先が中身を決めるファイルです)。ページ単位の窓(既定 5 ページ)で、総ページ数と続きの有無を返します。取引先から届く請求書のほとんどが PDF だからです。文字が取り出せないときは理由を分けて伝えます。紙を撮った PDF で文字が 1 つも無いのか、文字はあるがフォントを読めないのか。表は読み順のテキストになるので、金額を数えるときは CSV に書いてから `csv_aggregate` を通してください。OCR はしません

### Changed

- **`ToolRejectionCode` に `not_read` が増えました。** 読んでいないファイルを置き換えようとしたために実行しなかった呼び出しを表します
- **`ToolDefinition` に `adds` が増えました。** 書き込む Tool が、ファイルの中身を置き換えるのではなく足すことを表します。省くと置き換える扱いです
- **読んでいないファイルを置き換えません。** 置き換える呼び出しが誰の承認も経ずに実行に進むとき、その Work がそのファイルを読んでいるか書いたものであることを確かめます。どちらでもなければ実行せず、先に読むよう返します。人が承認した呼び出し(画面に差分が出ます)と、まだ無いファイルと、`adds` の Tool は対象外です
- **`ToolRejectionCode` に `base_changed` が増えました。** 承認を待つ間に書き込む先のファイルが変わったために実行しなかった呼び出しを表します。`Record<ToolRejectionCode, ...>` を書いている実装は追加してください

### Fixed

- 承認を待つ間に書き込む先のファイルが変わったとき、承認しても実行しません。人は画面で見た差分に対して承認するので、その差分の前提が変わったなら書かないほうが正しいためです。これまでは、決める間に自分でそのファイルを編集しても、同期サービスが新しい版を落としても、承認した内容がそのまま上書きしていました。判定のときのファイルの sha256 を承認の記録に残し、実行の直前に読み直して比べます
