...
- 承認、権限、複数人
- MCP 経由のセッション。外部のエージェントは自分の会話を持つ
- 端末の外への通知
...
- system prompt は Work の投影と同じ system prompt(職種の指示、会社、principal)に、担当の役割の段落を足す。人と話す。作業は `work_run` に出す。objective は人の言葉で書き、前提を添える。ファイルは自分で触らない。ファイルの中身を見ないと答えられない質問も Work にして調べる。結果は要約して伝える。件数や金額は Work の結果の数字のまま書き、計算し直さない。返答は端末に出るので Markdown の記法と絵文字は使わない
...
- Ink で描く。上から、会話(人の発言、担当の返答、子 Work の進捗行と締めの行(状態、書いたファイル、使用量。要約は担当が伝える)、質問)、状態行(会社名、model、進行中の Work、セッションの使用量)、入力欄
