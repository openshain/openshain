...
会話は Work の記録から毎ターン組み立て直すので、話すほど model へ送る入力が増えます。いまの版には減らす仕組みが 1 つしかありません。作業の Work を閉じたとき、その Work の Tool の結果を要約 1 行に畳むことです(`foldAway`)。人の発言と社員エージェントの返答は増え続けます。

いずれ model の窓に入らなくなり、そのときは呼び出しが失敗します。**同じ投影を次のターンでも作るので、そのセッションは以後ずっと失敗します**。閉じて開き直すほかありません。次の版でここを塞ぎます。

決めてあること。

- 圧縮は client の仕事です。Runtime には入れません
- 要約は `conversation.compacted` として**記録に残します**。どこまでを要約したか(seq)、要約の本文、使ったモデルと使用量を持ちます
- 投影は、最後の `conversation.compacted` の要約を先頭に置き、そこから後のイベントを続けます。記録は書き換えません。**同じ記録から同じ投影ができる**という規則は保たれます
- 自動と手動の両方を持ちます。入力の見積もりが閾値を超えたターンの前に自動で行い、`/compact` で人からも実行できます
- 圧縮の前後で会話が続くこと、記録から同じ投影ができることを、テストと実モデルの両方で確かめます
