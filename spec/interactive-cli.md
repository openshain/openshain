...
- 会社の決まりを引いたターンで、返答がどの決まりの id も含まないとき、client は「参照した会社の決まり」として、引いた決まりの id と有効日(多くとも 3 件)を返答の末尾に追加します。「見つかりません」と答えたターンには追加しません。どの決まりを引いたかは記録にある事実なので、返答の書きぶりに委ねません
...
会話は Work の記録から毎ターン組み立て直すので、話すほどモデルへ送る入力が増えます。減らす仕組みは 1 つしかありません。作業の Work を閉じたとき、その Work の Tool の結果を要約 1 行に畳むことです(`foldAway`)。人の発言と社員エージェントの返答は増え続けます。

いずれモデルの窓に入らなくなり、そのときは呼び出しが失敗します。**同じ投影を次のターンでも作るので、そのセッションは以後ずっと失敗します**。閉じて開き直すほかありません。ここを塞ぎます。

圧縮は client の仕事です。Runtime は圧縮を行わず、記録を受け取るだけです。

### 記録と投影

- 要約は `conversation.compacted` として記録に残します。payload は、要約した最後のイベントの `seq`、要約の本文、使ったモデル、使用量です。セッションの Work に記録します
- 投影は、最後の `conversation.compacted` を探し、その本文を会話の先頭に置き、その `seq` より後のイベントを続けます。**記録は書き換えません**。同じ記録から同じ投影ができるという規則は保たれます
- 圧縮の呼び出し自体も、セッションの Work に `model.requested`、`model.completed`、`usage.recorded` として残ります。使用量は `work show` の合計に入ります
- 作業の Work の中は圧縮しません。閉じたときに畳む仕組みが既にあります

### いつ行うか

- 直前のモデル呼び出しが実際に使った入力トークン(`model.completed` の usage)が閾値を超えていたら、**次のターンを始める前に** 1 回だけ圧縮します。見積もりではなく実測値を使うのは、トークンの数え方がモデルごとに違うからです
- 閾値は `limits.compact_at_input_tokens`(既定 120000)です。0 で圧縮しません
- `/compact` で人からも実行できます。閾値に届いていなくても実行します

### 要約に何を残すか

圧縮用の指示と、そこまでの投影を同じモデルに 1 回渡します。要約に残すのは、いま進んでいる依頼、決まったこと、書いたファイルの場所、開いている Work の id と状態、引いた会社の決まりの id、人が伝えた前提、未解決の質問です。落とすのは、Tool が返した生の内容と、閉じた Work の細部です。

要約はモデルの出力なので、会話の文脈にしか使いません。権限の判定、Work の状態、記録の中身は要約から作らず、これまでどおり記録そのものから作ります。

### 失敗したとき

圧縮の呼び出しが失敗したら、圧縮せずにそのターンを続け、画面に注意を 1 行表示します。窓を超えて失敗する場合の挙動は、いまと同じです。圧縮の失敗で会話を止めません。

### 完了の条件

1. 閾値を超えたセッションで、次のターンの前に 1 回だけ圧縮が走り、会話がそのまま続くこと。同じターンで 2 回走らないこと
2. `conversation.compacted` が記録に残り、その記録を読み直した投影が、圧縮の直後の投影と一致すること
3. `/compact` で人から実行でき、画面に 1 行表示されること。要約の後も直前の話題が続くこと
4. 圧縮の呼び出しが失敗したとき、ターンは続き、注意が 1 行表示されること
5. 圧縮後の投影に、開いている Work の id、書いたファイルの場所、引いた決まりの id が残ること
6. 圧縮の使用量が `work show` の合計に入ること
7. 圧縮をまたいで前の話題を引き継げることを、小さいモデルと大きいモデルの両方で確かめること(`OPENSHAIN_LIVE_TESTS=1`)
