...
- Ink で描きます。会話(人の発言、社員エージェントの返答、子 Work の進捗行と締めの行(状態、書いたファイル、使用量。要約は社員エージェントが伝えます)、質問)は端末に流して残し、端末のスクロールで遡れます。状態行(会社名、model、進行中の Work、セッションの使用量)と入力欄は画面の下に固定します
