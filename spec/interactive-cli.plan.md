...
`human.message` イベント、`work.created` の `parent`、投影が `human.message` を user message にすること。予約名に `work_run` と `work_show` を足します。`WorkStore.create` が `parent` を受けます。schema を生成し直します。
...
Ink の画面です。会話、状態行、入力欄。スラッシュコマンド。Ctrl-C。`bin.ts` は引数なしで端末なら画面を出します。

- 受け入れ: ink-testing-library で、発言と返答と子 Work の進捗が並び、`/work list` と `/tools` が既存の関数の出力を出し、`/quit` で閉じます。端末でなければ使い方を出します(既存のテスト)
