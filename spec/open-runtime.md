...
- system: profession の指示文、会社名、principal、Runtime の決まり(件数、合計、検索は Tool の値をそのまま使う。末尾の残量の 1 行は通知で返事は要らない。依頼が終わったら要約して終える)です
...
| `openshain init` | カレントディレクトリに `openshain.yaml`、`.mcp.json`、`AGENTS.md`、`CLAUDE.md` のひな型を書きます。`openshain.yaml` があれば断り、他の 3 つは無いものだけ書きます |
