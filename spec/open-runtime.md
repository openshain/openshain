...
| `csv_append` | mutate | `path`、`rows` | 足した行数、足した後の全行数、path と sha256(`after`)。列は既にあるファイルの見出しに従います |
...
| `excel_read` | observe | `path`、`sheet`(名前。省略すると最初のシート)、`offset`(行)、`limit`(既定 50 行) | シートの名前の一覧、列名、全行数、範囲の行(1 行 1 オブジェクト)、続きがあるか |
| `pdf_read` | observe | `path`、`offset`(ページ)、`limit`(既定 5 ページ) | 範囲のページの本文(ページごと)と、全ページ数、続きがあるか。文字だけを取り出します |
...
- `excel_read` は `.xlsx` を読みます。1 行目を列名にし、`csv_read` と同じ形の行を返します。列名が空のセルと、既に使われている名前は、その列の記号(A、B、C)を名前にします。名前が重なって行の値が消えることが無いようにするためです。日付のセルは `YYYY-MM-DD` にします(Excel は日付を数値で持つので、そのまま渡すと `46204` のような数になります)。時差で 1 日ずれないよう UTC で書きます。数式のセルは、Excel が保存した計算結果を返します。古い `.xls`(バイナリ)は読めないと返します。読む前に zip の中の宣言サイズを確かめ、展開が大きすぎるファイルは開きません(1 MiB に満たない `.xlsx` が数百 MiB に展開されることがあり、取引先が中身を決めるファイルです)。
- `csv_append` は既にある CSV に行を足します。無いファイルには足さず、`csv_write` で作るよう返します。列は既にある見出しに従い、見出しに無い列を持つ行は足さずに拒否します(黙って落とすと、書いたつもりのデータが消えます)。見出しにあって行に無い列は空欄です。全文を書き直させないための Tool です。1 回の返答で書ける量には model 側の上限があり、数十行を超える表を 1 回で書こうとするとそこに当たります。
- 会社フォルダのファイルを**置き換える**呼び出しが、誰の承認も経ずに実行に進むときは、その Work がそのファイルを読んでいるか、その Work が書いたものであることを確かめます。どちらでもなければ実行せず、`tool.rejected`(`not_read`)を残して「先に読むこと」と返します。人が承認した呼び出しには当てはまりません。承認の画面には差分が出るので、人はそのファイルの中身を見ています。まだ無いファイルにも当てはまりません。失うものが無いからです。`ToolDefinition` に `adds: true` と書いた Tool(`csv_append`)も、置き換えないので当てはまりません。読んだことは記録の `observation`、書いたことは `after` から判定します。
- `pdf_read` は PDF の文字だけを取り出します。画像は読まず、OCR もしません。表は読み順のテキストになるので、金額の合計は CSV に落としてから `csv_aggregate` を通します。読めないときの返事は 2 つに分かれます。文字が 1 つも無い PDF(画像として取り込んだもの)と、文字はあるがフォントを読めない PDF です。前者は元の書類を文字の入った PDF でもらうしかなく、後者は openshain の側の限界なので、区別して伝えます。PDF は 20 MiB まで読みます(本文のテキストだけが model に渡るので、1 MiB のファイルの上限とは別に持ちます)。
...
client                                  Runtime(MCP Tool)
  ...
  投影を組み立て、モデルを呼ぶ
  (work_record: model.requested、model.completed、usage.recorded、conversation.compacted)
...
| `work_select` | 既存の Work を現在の Work にし、`history` 付きで返します。終わった Work と、設定の principal と違う人の Work は受け付けません |
...
- `work_create` は Work を作って `in_progress` にします(理由は「an agent took the work over MCP」)。`work_select` は終わった Work と、その Work の principal が設定の principal と違うものを受け付けません。別の人の会話に client がイベントを書く経路を残さないためです(`work_record` は選んだ Work にしか書けません)。読み取り(`work_get`、`work_list`)はこの版では会社フォルダのどの Work も返します。`work_get` は id を省くと現在の Work です
