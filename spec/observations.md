...
- `payloadRef` は中身の在りかを指す**不透明な文字列**です。会社フォルダの中のパスでも、メールの message-id でも、外のシステムの id でも構いません。中身そのものは観測に入れません。メールの全文を事実の記録に埋め込むと、記録が中身の複製になります。**これをファイルの名前として開く側は、ほかのパスと同じ guard を通してください。** 観測を書くのは外の仕組みなので、ここに書かれた文字列は会社フォルダの中を指しているとは限りません
